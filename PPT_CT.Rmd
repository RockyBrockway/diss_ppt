...
cols_to_check <- c("LengthAxial", "LengthMax", "WidthMax", "WidthBasal", "WidthNeck", "Thickness", "DSA", "PSA")  
...
# Apply the normality test to each trait within each PointType3 group
normality_results <- data %>%
  group_by(PointType3) %>%
  summarise(across(2:9, ~test_normality(.), .names = "p_value_{col}"))
...
# Filter the data for the selected point types
filtered_data <- data %>%
  filter(PointType3 %in% c("Elko Series", "Gatecliff Series", "Rosegate Series"))
